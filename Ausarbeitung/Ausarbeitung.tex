...
\begin{figure}[H] 
    \centering
    \subfloat{{\includegraphics[width=0.69\textwidth]{HilbertToHilbert} }}%
    \caption{Bottom-up Errechnung der Koordinaten eines Punkts der Hilbert Kurve}%
...
Dieser Ansatz kann zwar keinen einzelnen Punkt an einem gegebenen Index liefern ohne die gesamte Kurve zu berechnen, andererseits ist dafür die Berechnung aller Punkte der Kurve insgesamt bei weitem effizienter als der iterative Aufruf des Punkt-zu-Punkt Ansatzes, da schon durchgeführte Berechnungen wiederverwertet werden. Algorithmus \ref{moore:batch} zeigt, wie zunächst die Hilbert Kurve bis Grad n-1 aufgebaut und daraus dann die Moore Kurve des Grads n erzeugt wird. Für die Nachvollziehbarkeit sind Abbildung \ref{HilbertZuHilbert} und \ref{HilbertZuMoore} essentiell.
...
\textbf{Hier: zeigen dass die Laufzeit pro punkt konstant ist bei unserer zweiten Method
